#!/usr/bin/env node
/**
 * dtb-session-care — session-log health check and repair (the "no more bricked
 * harness because of one bad log" plugin).
 *
 * Usage:
 *   dtb-session-care health [--root <dir>] [--session <id>] [--json]
 *   dtb-session-care repair [--root <dir>] [--session <id>] [--apply] [--json]
 *
...
const HELP = `dtb-session-care — DeepSeek Harness 工具集（dsh-toolbox）· 会话日志健康检查与修复
  ...
  if (command === 'health') {
    ...
    if (flags.json === true) {
      ...
      console.log(bad === 0
        ...
        : `\n发现 ${bad} 个问题日志；运行 "dtb-session-care repair --apply" 尝试修复。`)
    }
    return
  }
...
main().catch((error) => {
  console.error(`dtb-session-care: ${error.message}`)
  process.exit(1)
})
